...
Highlight_Words = (body,regex) => { //body == Email_Body <.a3s> 
    ...
    const Tree_Walker = documents.createTreeWalker(body, NodeFilter.SHOW_TEXT, { acceptNode: (node) => {
        
        if (!node.parentNode ||node.textContent?.trim()){  //if no parent or epty reject the mandem
    ...
    let Tree_Temp_Nodes = [];  //temporary arary to store the nodes mid traversal
                                // using tree walker if me modify gmail DOM it will break#
                                //remever node is for example </p> node<p> and not the whole email body
    ...
    Tree_Temp_Nodes.forEach(node => {
        console.log("Node:", node); //TEST remove this
        if (!regex || !node.textContent) {
            return;
        }
...
chrome.runtime.onMessage.addListener((Page_ID, Message, sendResponse) => {
